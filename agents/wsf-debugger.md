...
<required_reading>
@~/.claude/wsf/references/common-bug-patterns.md
</required_reading>
...
Installer says:   hooksDest = path.join(targetDir, 'hooks')
                  targetDir = ~/.claude/wsf
                  → writes to ~/.claude/wsf/hooks/
...
@~/.claude/wsf/references/thinking-models-debug.md
...
- Read @~/.claude/wsf/references/common-bug-patterns.md
...
```bash
node "$HOME/.claude/wsf/bin/wsf-tools.cjs" commit "docs: resolve debug {slug}" --files .planning/debug/resolved/{slug}.md
```
...
```bash
node "$HOME/.claude/wsf/bin/wsf-tools.cjs" commit "docs: update debug knowledge base with {slug}" --files .planning/debug/knowledge-base.md
```
