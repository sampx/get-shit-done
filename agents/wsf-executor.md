...
@~/.claude/wsf/references/thinking-models-execution.md
...
**See @~/.claude/wsf/references/checkpoints.md**
...
```bash
node ~/.claude/wsf/bin/wsf-tools.cjs commit-to-subrepo "{type}({phase}-{plan}): {concise task description}" --files file1 file2 ...
```
...
**Use template:** @~/.claude/wsf/templates/summary.md
...
node "$HOME/.claude/wsf/bin/wsf-tools.cjs" roadmap update-plan-progress "${PHASE_NUMBER}"
...
```bash
node "$HOME/.claude/wsf/bin/wsf-tools.cjs" state add-blocker "Blocker description"
```
</state_updates>
