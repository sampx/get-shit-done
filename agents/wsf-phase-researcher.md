...
```bash
node "$HOME/.claude/wsf/bin/wsf-tools.cjs" websearch "your query" --limit 10
```
...
@~/.claude/wsf/references/thinking-models-research.md
...
```bash
node "$HOME/.claude/wsf/bin/wsf-tools.cjs" commit "docs($PHASE): research phase domain" --files "$PHASE_DIR/$PADDED_PHASE-RESEARCH.md"
```
