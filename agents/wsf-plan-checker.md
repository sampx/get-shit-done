...
@~/.claude/wsf/references/few-shot-examples/plan-checker.md
...
for plan in "$PHASE_DIR"/*-PLAN.md; do
  echo "=== $plan ==="
  PLAN_STRUCTURE=$(node "$HOME/.claude/wsf/bin/wsf-tools.cjs" verify plan-structure "$plan")
  echo "$PLAN_STRUCTURE"
done
```
...
```bash
MUST_HAVES=$(node "$HOME/.claude/wsf/bin/wsf-tools.cjs" frontmatter get "$PLAN_PATH" --field must_haves)
```
...
```bash
PLAN_STRUCTURE=$(node "$HOME/.claude/wsf/bin/wsf-tools.cjs" verify plan-structure "$PLAN_PATH")
```
