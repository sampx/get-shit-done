...
Use template from `~/.claude/wsf/templates/UI-SPEC.md`.
...
Read template: `~/.claude/wsf/templates/UI-SPEC.md`
...
```bash
node "$HOME/.claude/wsf/bin/wsf-tools.cjs" commit "docs($PHASE): UI design contract" --files "$PHASE_DIR/$PADDED_PHASE-UI-SPEC.md"
```
