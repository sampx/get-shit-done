...
Intel system is disabled. To activate:
  ...
  node $HOME/.claude/wsf/bin/wsf-tools.cjs config-set intel.enabled true
...
```bash
node $HOME/.claude/wsf/bin/wsf-tools.cjs intel query <term>
```
...
```bash
node $HOME/.claude/wsf/bin/wsf-tools.cjs intel status
```
...
```bash
node $HOME/.claude/wsf/bin/wsf-tools.cjs intel diff
```
...
Project root: ${CWD}
wsf-tools path: $HOME/.claude/wsf/bin/wsf-tools.cjs
