...
// Check for WSF updates in background, write result to cache
...
function detectConfigDir(baseDir) {
  ...
  if (envDir && fs.existsSync(path.join(envDir, 'wsf', 'VERSION'))) {
    return envDir;
  }
  for (const dir of ['.claude', '.gemini', '.config/kilo', '.kilo', '.config/opencode', '.opencode']) {
    if (fs.existsSync(path.join(baseDir, dir, 'wsf', 'VERSION'))) {
...
const cacheDir = path.join(homeDir, '.cache', 'wsf');
const cacheFile = path.join(cacheDir, 'wsf-update-check.json');

// VERSION file locations (check project first, then global)
const projectVersionFile = path.join(projectConfigDir, 'wsf', 'VERSION');
const globalVersionFile = path.join(globalConfigDir, 'wsf', 'VERSION');
...
const child = spawn(process.execPath, ['-e', `
  ...
  // Only check hooks that WSF currently ships — orphaned files from removed features
  // (e.g., wsf-intel-*.js) must be ignored to avoid permanent stale warnings (#1750)
  const MANAGED_HOOKS = [
    'wsf-check-update.js',
      ...
      if (fs.existsSync(hooksDir)) {
        ...
        for (const hookFile of hookFiles) {
          try {
            ...
            const versionMatch = content.match(/\\/\\/ wsf-hook-version:\\s*(.+)/);
            if (versionMatch) {
  ...
  let latest = null;
  try {
    latest = execSync('npm view wsf-cc version', { encoding: 'utf8', timeout: 10000, windowsHide: true }).trim();
  } catch (e) {}
