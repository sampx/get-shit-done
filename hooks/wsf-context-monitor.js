...
process.stdin.on('end', () => {
  ...
  try {
    ...
    // Quick sentinel check: skip config read entirely for non-WSF projects (#P2.5).
    ...
    // Detect if WSF is active (has .planning/STATE.md in working directory)
    const isWsfActive = fs.existsSync(path.join(cwd, '.planning', 'STATE.md'));
    ...
    if (isCritical) {
      message = isWsfActive
        ? `CONTEXT CRITICAL: Usage at ${usedPct}%. Remaining: ${remaining}%. ` +
          ...
          'WSF state is already tracked in STATE.md. Inform the user so they can run ' +
          '/wsf-pause-work at the next natural stopping point.'
        : `CONTEXT CRITICAL: Usage at ${usedPct}%. Remaining: ${remaining}%. ` +
