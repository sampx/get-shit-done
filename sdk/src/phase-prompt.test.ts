...
async function createTempDir(): Promise<string> {
  return mkdtemp(join(tmpdir(), 'wsf-prompt-'));
}
...
describe('PromptFactory', () => {
  ...
  describe('buildPrompt', () => {
    it('assembles research prompt with role + purpose + process + context', async () => {
      ...
      await writeFile(
        join(agentsDir, 'wsf-phase-researcher.md'),
        makeAgentDef('wsf-phase-researcher', 'Read, Grep, Bash', 'You are a researcher.'),
      );
    ...
    it('assembles plan prompt with all context files', async () => {
      ...
      await writeFile(
        join(agentsDir, 'wsf-planner.md'),
        makeAgentDef('wsf-planner', 'Read, Write, Bash', 'You are a planner.'),
      );
    ...
    it('delegates execute phase with plan to buildExecutorPrompt', async () => {
      await writeFile(
        join(agentsDir, 'wsf-executor.md'),
        makeAgentDef('wsf-executor', 'Read, Write, Edit, Bash', 'You are an executor.'),
      );
    ...
    it('assembles verify prompt with phase instructions', async () => {
      ...
      await writeFile(
        join(agentsDir, 'wsf-verifier.md'),
        makeAgentDef('wsf-verifier', 'Read, Bash, Grep', 'You are a verifier.'),
      );
    ...
    it('handles missing workflow file gracefully', async () => {
      ...
      await writeFile(
        join(agentsDir, 'wsf-phase-researcher.md'),
        makeAgentDef('wsf-phase-researcher', 'Read, Bash', 'You are a researcher.'),
      );
  ...
  describe('loadAgentDef', () => {
    ...
    it('falls back to project agents dir', async () => {
      ...
      await writeFile(
        join(projectAgentsDir, 'wsf-executor.md'),
    ...
    it('prefers user agents dir over project agents dir', async () => {
      const projectAgentsDir = join(tempDir, 'project-agents');
      await mkdir(projectAgentsDir, { recursive: true });
      await writeFile(join(agentsDir, 'wsf-executor.md'), 'user agent');
      await writeFile(join(projectAgentsDir, 'wsf-executor.md'), 'project agent');
  ...
  describe('headless prompt loading', () => {
    it('loadWorkflowFile prefers sdkPromptsDir over WSF-1 workflowsDir', async () => {
      const sdkDir = join(tempDir, 'sdk-prompts');
      await mkdir(join(sdkDir, 'workflows'), { recursive: true });

      // Write both: WSF-1 original and SDK headless version
      await writeFile(join(workflowsDir, 'research-phase.md'), 'WSF-1 original');
    ...
    it('loadWorkflowFile falls back to WSF-1 when sdkPromptsDir file missing', async () => {
      const sdkDir = join(tempDir, 'sdk-prompts');
      await mkdir(join(sdkDir, 'workflows'), { recursive: true });

      // Only WSF-1 original exists, no SDK version
      await writeFile(join(workflowsDir, 'research-phase.md'), 'WSF-1 original');
      ...
      const content = await factory.loadWorkflowFile(PhaseType.Research);
      expect(content).toBe('WSF-1 original');
    });
    ...
    it('loadAgentDef prefers sdkPromptsDir over user agents dir', async () => {
      ...
      await writeFile(join(agentsDir, 'wsf-executor.md'), 'user agent');
      await writeFile(join(sdkDir, 'agents', 'wsf-executor.md'), 'SDK headless agent');
    ...
    it('buildPrompt sanitizes interactive patterns from output', async () => {
      ...
      expect(prompt).not.toContain('/wsf-');
