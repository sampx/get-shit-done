...
const WORKFLOW_PATH = path.join(__dirname, '..', 'wsf', 'workflows', 'autonomous.md');
...
describe('autonomous workflow ui-phase and ui-review integration (#1375)', () => {
  ...
  describe('step 3a.5 — UI design contract before planning', () => {
    ...
    test('UI design contract step invokes wsf-ui-phase skill', () => {
      assert.ok(
        content.includes('skill="wsf-ui-phase"'),
        'should invoke wsf-ui-phase via Skill()'
      );
    });
  ...
  describe('step 3d.5 — UI review after execution', () => {
    ...
    test('UI review step invokes wsf-ui-review skill', () => {
      assert.ok(
        content.includes('skill="wsf-ui-review"'),
        'should invoke wsf-ui-review via Skill()'
      );
    });
