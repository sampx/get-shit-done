 ...
 *
 * wsf-tools must reject unknown/invalid flags (--help, -h, etc.) with a
...
describe('unknown flag guard (bug #1818)', () => {
  ...
  test('phases clear --help is rejected with non-zero exit', () => {
    const result = runWsfTools(['phases', 'clear', '--help'], tmpDir);
  ...
  test('generate-slug hello --help is rejected', () => {
    ...
    const ok = runWsfTools(['generate-slug', 'hello'], tmpDir);
    assert.strictEqual(ok.success, true, 'control: generate-slug without --help must succeed');

    const result = runWsfTools(['generate-slug', 'hello', '--help'], tmpDir);
    assert.strictEqual(result.success, false);
    assert.match(result.error, /--help/);
  });

  test('phase complete --help is rejected', () => {
    const result = runWsfTools(['phase', 'complete', '--help'], tmpDir);
    assert.strictEqual(result.success, false);
    assert.match(result.error, /--help/);
  });

  test('state load --help is rejected', () => {
    const result = runWsfTools(['state', 'load', '--help'], tmpDir);
    assert.strictEqual(result.success, false);
    assert.match(result.error, /--help/);
  });
  ...
  test('phases clear -h is rejected', () => {
    const result = runWsfTools(['phases', 'clear', '-h'], tmpDir);
    assert.strictEqual(result.success, false);
    assert.match(result.error, /-h/);
  });

  test('generate-slug hello -h is rejected', () => {
    const result = runWsfTools(['generate-slug', 'hello', '-h'], tmpDir);
    assert.strictEqual(result.success, false);
    assert.match(result.error, /-h/);
  });
  ...
  test('generate-slug hello --version is rejected', () => {
    const result = runWsfTools(['generate-slug', 'hello', '--version'], tmpDir);
    assert.strictEqual(result.success, false);
    assert.match(result.error, /--version/);
  });

  test('current-timestamp --help is rejected', () => {
    const result = runWsfTools(['current-timestamp', '--help'], tmpDir);
    assert.strictEqual(result.success, false);
    assert.match(result.error, /--help/);
  });
});
