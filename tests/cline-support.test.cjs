...
describe('Cline runtime support', () => {
  ...
  test('.clinerules references WSF workflow enforcement', () => {
    const content = fs.readFileSync(path.join(__dirname, '..', '.clinerules'), 'utf-8');
    assert.ok(
      content.includes('wsf') || content.includes('WSF') || content.includes('workflow'),
      '.clinerules should mention WSF workflows'
    );
  });
