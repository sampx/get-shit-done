/**
 * WSF Tools Tests - Dispatcher
 *
 * Tests for wsf-tools.cjs dispatch routing and error paths.
...
describe('dispatcher error paths', () => {
  ...
  test('no-command invocation prints usage and exits non-zero', () => {
    const result = runWsfTools('', tmpDir);
  ...
  test('unknown command produces clear error and exits non-zero', () => {
    const result = runWsfTools('nonexistent-cmd', tmpDir);
    assert.strictEqual(result.success, false, 'Should exit non-zero');
  ...
  test('--cwd= form overrides working directory', () => {
    ...
    const result = runWsfTools(`--cwd=${tmpDir} state load`, process.cwd());
  ...
  test('--cwd= with empty value produces error', () => {
    const result = runWsfTools('--cwd= state load', tmpDir);
    assert.strictEqual(result.success, false, 'Should exit non-zero');
  ...
  test('--cwd with invalid path produces error', () => {
    const result = runWsfTools('--cwd /nonexistent/path/xyz state load', tmpDir);
    assert.strictEqual(result.success, false, 'Should exit non-zero');
  ...
  test('frontmatter unknown subcommand errors', () => {
    const result = runWsfTools('frontmatter bogus file.md', tmpDir);
    assert.strictEqual(result.success, false, 'Should exit non-zero');
  ...
  test('verify unknown subcommand errors', () => {
    const result = runWsfTools('verify bogus', tmpDir);
    assert.strictEqual(result.success, false, 'Should exit non-zero');
  ...
  test('phases unknown subcommand errors', () => {
    const result = runWsfTools('phases bogus', tmpDir);
    assert.strictEqual(result.success, false, 'Should exit non-zero');
  ...
  test('roadmap unknown subcommand errors', () => {
    const result = runWsfTools('roadmap bogus', tmpDir);
    assert.strictEqual(result.success, false, 'Should exit non-zero');
  ...
  test('requirements unknown subcommand errors', () => {
    const result = runWsfTools('requirements bogus', tmpDir);
    assert.strictEqual(result.success, false, 'Should exit non-zero');
  ...
  test('phase unknown subcommand errors', () => {
    const result = runWsfTools('phase bogus', tmpDir);
    assert.strictEqual(result.success, false, 'Should exit non-zero');
  ...
  test('milestone unknown subcommand errors', () => {
    const result = runWsfTools('milestone bogus', tmpDir);
    assert.strictEqual(result.success, false, 'Should exit non-zero');
  ...
  test('validate unknown subcommand errors', () => {
    const result = runWsfTools('validate bogus', tmpDir);
    assert.strictEqual(result.success, false, 'Should exit non-zero');
  ...
  test('todo unknown subcommand errors', () => {
    const result = runWsfTools('todo bogus', tmpDir);
    assert.strictEqual(result.success, false, 'Should exit non-zero');
  ...
  test('uat unknown subcommand errors', () => {
    const result = runWsfTools('uat bogus', tmpDir);
    assert.strictEqual(result.success, false, 'Should exit non-zero');
  ...
  test('init unknown workflow errors', () => {
    const result = runWsfTools('init bogus', tmpDir);
    assert.strictEqual(result.success, false, 'Should exit non-zero');
...
describe('dispatcher routing branches', () => {
  ...
  test('init resume returns valid JSON', () => {
    ...
    const result = runWsfTools('init resume', tmpDir);
  ...
  test('init verify-work returns valid JSON', () => {
    ...
    const result = runWsfTools('init verify-work 01', tmpDir);
  ...
  test('state with no subcommand calls cmdStateLoad', () => {
    ...
    const result = runWsfTools('state', tmpDir);
