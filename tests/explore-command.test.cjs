...
describe('explore command', () => {
  test('command file exists', () => {
    const p = path.join(__dirname, '..', 'commands', 'wsf', 'explore.md');
    assert.ok(fs.existsSync(p), 'commands/wsf/explore.md should exist');
  });

  test('command file has required frontmatter', () => {
    const p = path.join(__dirname, '..', 'commands', 'wsf', 'explore.md');
    const content = fs.readFileSync(p, 'utf-8');
    assert.ok(content.includes('name: wsf-explore'), 'Command must have name frontmatter');
  ...
  test('workflow file exists', () => {
    const p = path.join(__dirname, '..', 'wsf', 'workflows', 'explore.md');
