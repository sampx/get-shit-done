/**
 * WSF Tools Tests — phase next-decimal ROADMAP.md scanning
 *
...
describe('phase next-decimal ROADMAP.md scanning (#1865)', () => {
  ...
  test('directory-only scan still works (existing behavior)', () => {
    ...
    const result = runWsfTools('phase next-decimal 999', tmpDir);
  ...
  test('handles missing ROADMAP.md gracefully', () => {
    ...
    const result = runWsfTools('phase next-decimal 5', tmpDir);
    assert.ok(result.success, `Command failed: ${result.error}`);
  ...
  test('handles empty phases dir with ROADMAP.md entries', () => {
    ...
    const result = runWsfTools('phase next-decimal 10', tmpDir);
    assert.ok(result.success, `Command failed: ${result.error}`);
  ...
  test('handles leading-zero phase numbers in ROADMAP.md', () => {
    ...
    const result = runWsfTools('phase next-decimal 7', tmpDir);
    assert.ok(result.success, `Command failed: ${result.error}`);
