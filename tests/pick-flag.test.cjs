...
describe('--pick flag', () => {
  test('extracts a top-level field from JSON output', () => {
    const result = runWsfTools('generate-slug "hello world" --pick slug');
  ...
  test('returns empty string for missing field', () => {
    const result = runWsfTools('generate-slug "test" --pick nonexistent');
    assert.strictEqual(result.success, true);
    assert.strictEqual(result.output, '');
  });

  test('errors when --pick has no value', () => {
    const result = runWsfTools('generate-slug "test" --pick');
    assert.strictEqual(result.success, false);
    assert.match(result.error, /Missing value for --pick/);
  });

  test('errors when --pick value starts with --', () => {
    const result = runWsfTools(['generate-slug', 'test', '--pick', '--raw']);
    assert.strictEqual(result.success, false);
    assert.match(result.error, /Missing value for --pick/);
  });
  ...
  test('does not collide with frontmatter --field flag', () => {
    ...
    const result = runWsfTools('generate-slug "test-value" --pick slug');
    assert.strictEqual(result.success, true);
    assert.strictEqual(result.output, 'test-value');
  });

  test('works with current-timestamp command', () => {
    const result = runWsfTools('current-timestamp --pick timestamp');
    assert.strictEqual(result.success, true);
