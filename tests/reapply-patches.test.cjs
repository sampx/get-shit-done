/**
 * WSF Tools Tests - reapply-patches backup logic
 *
...
function createTempDir() {
  return fs.mkdtempSync(path.join(require('os').tmpdir(), 'wsf-patch-test-'));
}
...
function simulateManifestAndPatch(configDir, files) {
  ...
  fs.writeFileSync(
    path.join(configDir, 'wsf-file-manifest.json'),
...
describe('saveLocalPatches — patch backup and pristine hash tracking (#1469)', () => {
  ...
  test('detects modified files and backs them up', () => {
    simulateManifestAndPatch(tmpDir, {
      original: {
        'wsf/workflows/execute-phase.md': '# Execute Phase\nOriginal content\n',
        'wsf/workflows/plan-phase.md': '# Plan Phase\nOriginal content\n',
      },
      modified: {
    ...
    assert.ok(result.includes('wsf/workflows/execute-phase.md'));

    // Verify backup exists
    const backupPath = path.join(tmpDir, 'wsf-local-patches', 'wsf/workflows/execute-phase.md');
  ...
  test('backup-meta.json includes pristine_hashes for three-way merge', () => {
    ...
    simulateManifestAndPatch(tmpDir, {
      original: {
        'wsf/workflows/execute-phase.md': originalContent,
      },
      modified: {
        'wsf/workflows/execute-phase.md': originalContent + '\n## Custom\n',
      },
    ...
    const metaPath = path.join(tmpDir, 'wsf-local-patches', 'backup-meta.json');
    ...
    assert.strictEqual(
      meta.pristine_hashes['wsf/workflows/execute-phase.md'],
      expectedHash,
  ...
  test('backup-meta.json includes from_version and from_manifest_timestamp', () => {
    simulateManifestAndPatch(tmpDir, {
      original: { 'wsf/workflows/test.md': 'original' },
      modified: { 'wsf/workflows/test.md': 'modified' },
    });

    saveLocalPatches(tmpDir);

    const meta = JSON.parse(fs.readFileSync(
      path.join(tmpDir, 'wsf-local-patches', 'backup-meta.json'), 'utf8'
    ));
  ...
  test('unmodified files are not backed up', () => {
    simulateManifestAndPatch(tmpDir, {
      original: {
        'wsf/workflows/a.md': 'content A',
        'wsf/workflows/b.md': 'content B',
      },
      // No modifications
    });
    ...
    assert.ok(!fs.existsSync(path.join(tmpDir, 'wsf-local-patches')), 'patches dir should not be created');
  });

  test('multiple modified files all get pristine hashes', () => {
    simulateManifestAndPatch(tmpDir, {
      original: {
        'wsf/workflows/a.md': 'original A',
        'wsf/workflows/b.md': 'original B',
        'wsf/workflows/c.md': 'original C',
      },
      modified: {
        'wsf/workflows/a.md': 'modified A',
        'wsf/workflows/b.md': 'modified B',
      },
    });
    ...
    assert.strictEqual(meta.pristine_hashes['wsf/workflows/a.md'], sha256('original A'));
    assert.strictEqual(meta.pristine_hashes['wsf/workflows/b.md'], sha256('original B'));
    // c.md should NOT have a pristine hash (it wasn't modified)
    assert.strictEqual(meta.pristine_hashes['wsf/workflows/c.md'], undefined);
  });
  ...
  test('returns empty array when manifest is malformed', () => {
    fs.writeFileSync(path.join(tmpDir, 'wsf-file-manifest.json'), 'not json');
    const result = saveLocalPatches(tmpDir);
    assert.strictEqual(result.length, 0);
  });
});
...
describe('reapply-patches workflow contract (#1469)', () => {
  test('workflow file contains critical invariant about never skipping backed-up files', () => {
    const workflowPath = path.join(__dirname, '..', 'commands', 'wsf', 'reapply-patches.md');
