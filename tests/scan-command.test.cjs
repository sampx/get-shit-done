...
describe('scan command', () => {
  test('command file exists with correct name and description', () => {
    const p = path.join(__dirname, '..', 'commands', 'wsf', 'scan.md');
    assert.ok(fs.existsSync(p), 'commands/wsf/scan.md should exist');
    const content = fs.readFileSync(p, 'utf-8');
    assert.ok(content.includes('name: wsf-scan'), 'Command must have name: wsf-scan');
  ...
  test('workflow file exists', () => {
    const p = path.join(__dirname, '..', 'wsf', 'workflows', 'scan.md');
    assert.ok(fs.existsSync(p), 'wsf/workflows/scan.md should exist');
  });
  ...
  test('workflow references wsf-codebase-mapper', () => {
    const p = path.join(__dirname, '..', 'wsf', 'workflows', 'scan.md');
    const content = fs.readFileSync(p, 'utf-8');
    assert.ok(content.includes('wsf-codebase-mapper'),
      'Workflow should reference the wsf-codebase-mapper agent');
  });
});
