...
describe('state-snapshot command', () => {
  ...
  test('missing STATE.md returns error', () => {
    const result = runWsfTools('state-snapshot', tmpDir);
...
`
  ...
  describe('--cwd override', () => {
    ...
    beforeEach(() => {
      outsideDir = fs.mkdtempSync(path.join(require('os').tmpdir(), 'wsf-test-outside-'));
    });
...
`
      ...
      const result = runWsfTools(`state-snapshot --cwd "${tmpDir}"`, outsideDir);
      assert.ok(result.success, `Command failed: ${result.error}`);
  ...
  test('returns error for invalid --cwd path', () => {
    ...
    const result = runWsfTools(`state-snapshot --cwd "${invalid}"`, tmpDir);
...
`
    ...
    const result = runWsfTools(['state', 'add-blocker', '--text', 'Waiting on vendor quote $1.00 before approval'], tmpDir);
    assert.ok(result.success, `Command failed: ${result.error}`);
...
`
    ...
    const result = runWsfTools(`state add-blocker --text-file "${blockerPath}"`, tmpDir);
    assert.ok(result.success, `Command failed: ${result.error}`);
...
describe('state json command', () => {
  ...
  test('missing STATE.md returns error', () => {
    const result = runWsfTools('state json', tmpDir);
    assert.ok(result.success, `Command should succeed: ${result.error}`);
...
`
    ...
    const output = JSON.parse(result.output);
    assert.strictEqual(output.wsf_state_version, '1.0', 'should have version 1.0');
...
`
    ...
    const output = JSON.parse(result.output);
    assert.strictEqual(output.wsf_state_version, '1.0', 'version from frontmatter');
...
`
    ...
    const result = runWsfTools('state update Status "Executing Plan 1"', tmpDir);
    assert.ok(result.success, `Command failed: ${result.error}`);

    const content = fs.readFileSync(path.join(tmpDir, '.planning', 'STATE.md'), 'utf-8');
    assert.ok(content.startsWith('---\n'), 'should start with frontmatter delimiter');
    assert.ok(content.includes('wsf_state_version: 1.0'), 'should have version field');
...
`
    ...
    const result = runWsfTools('state patch --Status "In progress" --"Current Plan" 04-02', tmpDir);
    assert.ok(result.success, `Command failed: ${result.error}`);
...
`
    ...
    runWsfTools('state update Status "Paused"', tmpDir);
...
`
    ...
    runWsfTools('state update "Current Plan" "03-03"', tmpDir);
...
`
    ...
    runWsfTools('state update Status "Executing Plan 5"', tmpDir);
...
const { stateExtractField, stateReplaceField, stateReplaceFieldWithFallback } = require('../wsf/bin/lib/state.cjs');
...
describe('cmdStateLoad (state load)', () => {
  ...
  test('returns config and state when STATE.md exists', () => {
    ...
    const result = runWsfTools('state load', tmpDir);
    assert.ok(result.success, `Command failed: ${result.error}`);
  ...
  test('returns raw key=value format with --raw flag', () => {
    ...
    const result = runWsfTools('state load --raw', tmpDir);
    assert.ok(result.success, `Command failed: ${result.error}`);
...
describe('cmdStateGet (state get)', () => {
  ...
  test('returns full content when no section specified', () => {
    ...
    const result = runWsfTools('state get', tmpDir);
    assert.ok(result.success, `Command failed: ${result.error}`);
  ...
  test('extracts bold field value', () => {
    ...
    const result = runWsfTools('state get Status', tmpDir);
    assert.ok(result.success, `Command failed: ${result.error}`);
  ...
  test('extracts markdown section content', () => {
    ...
    const result = runWsfTools('state get Blockers', tmpDir);
    assert.ok(result.success, `Command failed: ${result.error}`);
  ...
  test('returns error for nonexistent field', () => {
    ...
    const result = runWsfTools('state get Missing', tmpDir);
...
describe('cmdStatePatch and cmdStateUpdate (state patch, state update)', () => {
  ...
  test('state patch reports failed fields that do not exist', () => {
    ...
    const result = runWsfTools('state patch --Status Done --Missing value', tmpDir);
    assert.ok(result.success, `Command failed: ${result.error}`);
  ...
  test('state update changes a single field', () => {
    ...
    const result = runWsfTools('state update Status "Phase complete"', tmpDir);
    assert.ok(result.success, `Command failed: ${result.error}`);
  ...
  test('state update reports field not found', () => {
    ...
    const result = runWsfTools('state update Missing value', tmpDir);
  ...
  test('state update returns error when STATE.md missing', () => {
    const result = runWsfTools('state update Status value', tmpDir);
...
describe('cmdStateRecordMetric (state record-metric)', () => {
  ...
  test('appends metric row to existing table', () => {
    ...
    const result = runWsfTools('state record-metric --phase 2 --plan 1 --duration 5min --tasks 3 --files 4', tmpDir);
    assert.ok(result.success, `Command failed: ${result.error}`);
  ...
  test('replaces None yet placeholder with first metric', () => {
    ...
    const result = runWsfTools('state record-metric --phase 1 --plan 1 --duration 2min --tasks 1 --files 2', tmpDir);
    assert.ok(result.success, `Command failed: ${result.error}`);
  ...
  test('returns error when required fields missing', () => {
    ...
    const result = runWsfTools('state record-metric --phase 1', tmpDir);
    assert.ok(result.success, `Command should exit 0: ${result.error}`);
  ...
  test('returns error when STATE.md missing', () => {
    const result = runWsfTools('state record-metric --phase 1 --plan 1 --duration 2min', tmpDir);
    assert.ok(result.success, `Command should exit 0: ${result.error}`);
...
describe('cmdStateUpdateProgress (state update-progress)', () => {
  ...
  test('calculates progress from plan/summary counts', () => {
    ...
    const result = runWsfTools('state update-progress', tmpDir);
    assert.ok(result.success, `Command failed: ${result.error}`);
...
describe('cmdStateResolveBlocker (state resolve-blocker)', () => {
  ...
  test('removes matching blocker line (case-insensitive substring match)', () => {
    ...
    const result = runWsfTools('state resolve-blocker --text "api credentials"', tmpDir);
    assert.ok(result.success, `Command failed: ${result.error}`);
  ...
  test('adds None placeholder when last blocker resolved', () => {
    ...
    const result = runWsfTools('state resolve-blocker --text "single blocker"', tmpDir);
    assert.ok(result.success, `Command failed: ${result.error}`);
  ...
  test('returns error when text not provided', () => {
    ...
    const result = runWsfTools('state resolve-blocker', tmpDir);
    assert.ok(result.success, `Command should exit 0: ${result.error}`);
  ...
  test('returns error when STATE.md missing', () => {
    const result = runWsfTools('state resolve-blocker --text "anything"', tmpDir);
    assert.ok(result.success, `Command should exit 0: ${result.error}`);
  ...
  test('returns resolved true even if no line matches', () => {
    ...
    const result = runWsfTools('state resolve-blocker --text "nonexistent blocker text"', tmpDir);
    assert.ok(result.success, `Command failed: ${result.error}`);
...
describe('cmdStateRecordSession (state record-session)', () => {
  ...
  test('updates Last session timestamp even with no other options', () => {
    ...
    const result = runWsfTools('state record-session', tmpDir);
    assert.ok(result.success, `Command failed: ${result.error}`);
  ...
  test('sets Resume file to None when not specified', () => {
    ...
    const result = runWsfTools('state record-session --stopped-at "Phase 1 complete"', tmpDir);
    assert.ok(result.success, `Command failed: ${result.error}`);
...
describe('milestone-scoped phase counting in frontmatter', () => {
  ...
  test('total_phases counts only current milestone phases', () => {
    ...
    const result = runWsfTools('state update Status "Executing"', tmpDir);
    assert.ok(result.success, `Command failed: ${result.error}`);

    // Read the state json to check frontmatter
    const jsonResult = runWsfTools('state json', tmpDir);
  ...
  test('without ROADMAP counts all phases (pass-all filter)', () => {
    ...
    const result = runWsfTools('state update Status "In progress"', tmpDir);
    assert.ok(result.success, `Command failed: ${result.error}`);
...
`;
    ...
    const adv1 = runWsfTools(['state', 'advance-plan'], tmpDir);
    ...
    const adv2 = runWsfTools(['state', 'advance-plan'], tmpDir);
...
describe('progress counters correct after plan execution (#1589)', () => {
  ...
  test('percent in frontmatter is derived from disk counts, not stale Progress body field', () => {
    ...
    const result = runWsfTools('state update Status "Milestone complete"', tmpDir);
    assert.ok(result.success, `Command failed: ${result.error}`);
...
`;
    ...
    const result = runWsfTools('phase complete 3', tmpDir);
...
`;
    ...
    const result = runWsfTools('phase complete 4', tmpDir);
    assert.ok(result.success, `phase complete failed: ${result.error}`);
...
`;
    ...
    runWsfTools('phase complete 5', tmpDir);
...
describe('state planned-phase command', () => {
  ...
  test('after call: Status is "Ready to execute"', () => {
    ...
    const result = runWsfTools(['state', 'planned-phase', '--phase', '3', '--name', 'API', '--plans', '5'], tmpDir);
    assert.ok(result.success, `Command failed: ${result.error}`);
  ...
  test('after call: Total Plans matches argument', () => {
    ...
    const result = runWsfTools(['state', 'planned-phase', '--phase', '2', '--name', 'Core', '--plans', '7'], tmpDir);
    assert.ok(result.success, `Command failed: ${result.error}`);
  ...
  test('after call: Last Activity is today\'s date', () => {
    ...
    const result = runWsfTools(['state', 'planned-phase', '--phase', '1', '--name', 'Setup', '--plans', '3'], tmpDir);
    assert.ok(result.success, `Command failed: ${result.error}`);
  ...
  test('missing STATE.md returns graceful error', () => {
    // No STATE.md written
    const result = runWsfTools(['state', 'planned-phase', '--phase', '1', '--name', 'Test', '--plans', '3'], tmpDir);
...
describe('state validate command', () => {
  ...
  test('STATE says executing + VERIFICATION.md shows passed emits warning', () => {
    ...
    const result = runWsfTools('state validate', tmpDir);
    assert.ok(result.success, `Command failed: ${result.error}`);
    const output = JSON.parse(result.output);
...
describe('state sync command', () => {
  ...
  test('drifted STATE.md + correct filesystem: after sync, fields match disk', () => {
    ...
    const result = runWsfTools('state sync', tmpDir);
    assert.ok(result.success, `Command failed: ${result.error}`);
    const output = JSON.parse(result.output);
  ...
  test('--verify flag reports changes without writing', () => {
    ...
    const result = runWsfTools('state sync --verify', tmpDir);
    assert.ok(result.success, `Command failed: ${result.error}`);
    const output = JSON.parse(result.output);
