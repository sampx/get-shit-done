 ...
 *
 * Regression tests for: https://github.com/wsf-build/wsf/issues/1343
 */
...
describe('@file: handoff in workflows', () => {
  test('all workflows calling wsf-tools init have @file: handler', () => {
    const workflowFiles = fs.readdirSync(WORKFLOWS_DIR)
      .filter(f => f.endsWith('.md'));
    ...
    for (const wf of workflowFiles) {
      ...
      // Check if this workflow calls wsf-tools.cjs init
      if (/INIT=\$\(node.*wsf-tools.*\binit\b/.test(content)) {
    ...
    assert.strictEqual(
      missing.length, 0,
      `Workflows calling wsf-tools init without @file: handler (large output will be truncated):\n  ${missing.join('\n  ')}`
    );
  });
});
