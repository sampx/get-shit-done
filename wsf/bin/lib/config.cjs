...
const VALID_CONFIG_KEYS = new Set([
  ...
  'gates.confirm_project', 'gates.confirm_phases', 'gates.confirm_roadmap', 'gates.confirm_breakdown', 'gates.confirm_plan', 'gates.execute_next_plan', 'gates.issues_review', 'gates.confirm_transition',
  'safety.always_confirm_destructive', 'safety.always_confirm_external_services',
  'parallelization.enabled', 'parallelization.plan_level', 'parallelization.task_level', 'parallelization.skip_checkpoints', 'parallelization.max_concurrent_agents', 'parallelization.min_plans_for_parallel',
 ...
 *   2. User-level defaults from ~/.wsf/defaults.json (if present)
 *   3. userChoices — the settings the user explicitly selected during /wsf-new-project
 *
...
function buildNewProjectConfig(userChoices) {
  ...
  // Load user-level defaults from ~/.wsf/defaults.json if available
  const globalDefaultsPath = path.join(homedir, '.wsf', 'defaults.json');
  let userDefaults = {};
  try {
 ...
 * configured during /wsf-new-project). All remaining keys are filled from
 * hardcoded defaults and optional ~/.wsf/defaults.json.
 *
