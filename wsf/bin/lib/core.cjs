...
/**
 * Remove stale wsf-* temp files/dirs older than maxAgeMs (default: 5 minutes).
 * Runs opportunistically before each new temp file write to prevent unbounded accumulation.
 * @param {string} prefix - filename prefix to match (e.g., 'wsf-')
 * @param {object} opts
 ...
 */
function reapStaleTempFiles(prefix = 'wsf-', { maxAgeMs = 5 * 60 * 1000, dirsOnly = false } = {}) {
  try {
...
function output(result, raw, rawValue) {
  ...
  if (raw && rawValue !== undefined) {
    ...
    if (json.length > 50000) {
      reapStaleTempFiles();
      const tmpPath = path.join(require('os').tmpdir(), `wsf-${Date.now()}.json`);
  ...
  try {
    ...
    if (unknownKeys.length > 0) {
      process.stderr.write(
        `wsf-tools: warning: unknown config key(s) in .planning/config.json: ${unknownKeys.join(', ')} — these will be ignored\n`
      );
    }
    ...
    // Fall back to ~/.wsf/defaults.json only for truly pre-project contexts (#1683)
    // If .planning/ exists, the project is initialized — just missing config.json
    if (fs.existsSync(planningDir(cwd))) {
      return defaults;
    }
    try {
      const home = process.env.WSF_HOME || os.homedir();
      const globalDefaultsPath = path.join(home, '.wsf', 'defaults.json');
 ...
 * Applied at write points so WSF-generated .planning/ files are IDE-friendly.
 *
 * Rules enforced:
 ...
 *
 * Resolution order:
 * 1. If WSF_PROJECT is set (env var or explicit `project` arg), routes to
 ...
 * 2. If WSF_WORKSTREAM is set, routes to `.planning/workstreams/{ws}/`.
 * 3. Otherwise returns `.planning/`.
 *
 * WSF_PROJECT and WSF_WORKSTREAM can be combined:
 *   `.planning/{project}/workstreams/{ws}/`
 *
 * @param {string} cwd - project root
 * @param {string} [ws] - explicit workstream name; if omitted, checks WSF_WORKSTREAM env var
 * @param {string} [project] - explicit project name; if omitted, checks WSF_PROJECT env var
 */
function planningDir(cwd, ws, project) {
  if (project === undefined) project = process.env.WSF_PROJECT || null;
  if (ws === undefined) ws = process.env.WSF_WORKSTREAM || null;

  // Reject path separators and traversal components in project/workstream names
  const BAD_SEGMENT = /[/\\]|\.\./;
  if (project && BAD_SEGMENT.test(project)) {
    throw new Error(`WSF_PROJECT contains invalid path characters: ${project}`);
  }
  if (ws && BAD_SEGMENT.test(ws)) {
    throw new Error(`WSF_WORKSTREAM contains invalid path characters: ${ws}`);
  }
 ...
 *
 * Order:
 * 1. Explicit runtime/session env vars (`WSF_SESSION_KEY`, `CODEX_THREAD_ID`, etc.)
...
function getSessionScopedWorkstreamFile(cwd) {
  ...
  const dirPath = path.join(os.tmpdir(), 'wsf-workstream-sessions', projectId);
  return {
    sessionKey,
    dirPath,
...
/**
 * Resolve the agents directory from the WSF install location.
 * wsf-tools.cjs lives at <configDir>/wsf/bin/wsf-tools.cjs,
 * so agents/ is at <configDir>/agents/.
 *
 * WSF_AGENTS_DIR env var overrides the default path. Used in tests and for
 * installs where the agents directory is not co-located with wsf-tools.cjs.
 *
 * @returns {string} Absolute path to the agents directory
 */
function getAgentsDir() {
  if (process.env.WSF_AGENTS_DIR) {
    return process.env.WSF_AGENTS_DIR;
  }
  // __dirname is wsf/bin/lib/ → go up 3 levels to configDir
  return path.join(__dirname, '..', '..', '..', 'agents');
}

/**
 * Check which WSF agents are installed on disk.
 * Returns an object with installation status and details.
 *
 * Recognises both standard format (wsf-planner.md) and Copilot format
 * (wsf-planner.agent.md). Copilot renames agent files during install (#1512).
 *
