 ...
 * with WSF marker detection, doc tooling detection, monorepo awareness, and
...
const WSF_MARKER = '<!-- generated-by: wsf-doc-writer -->';
...
/**
 * Check whether a file begins with the WSF doc writer marker.
 * Reads the first 500 bytes only — avoids loading large files.
 *
 * @param {string} filePath - Absolute path to the file
 * @returns {boolean}
 */
function hasWsfMarker(filePath) {
  try {
    const buf = Buffer.alloc(500);
    const fd = fs.openSync(filePath, 'r');
    const bytesRead = fs.readSync(fd, buf, 0, 500, 0);
    fs.closeSync(fd);
    return buf.slice(0, bytesRead).toString('utf-8').includes(WSF_MARKER);
  } catch {
    return false;
  }
}
 ...
 * @returns {Array<{path: string, has_wsf_marker: boolean}>}
 */
    ...
    try {
      ...
      for (const entry of entries) {
        ...
        if (entry.isDirectory()) {
          ...
          results.push({ path: rel, has_wsf_marker: hasWsfMarker(abs) });
        }
      }
 ...
 *
 * @example
 * node wsf-tools.cjs docs-init --raw
 *
...
function cmdDocsInit(cwd, raw) {
  ...
  const result = {
    doc_writer_model: resolveModelInternal(cwd, 'wsf-doc-writer'),
