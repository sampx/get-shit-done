...
const CLAUDE_MD_FALLBACKS = {
  project: 'Project not yet initialized. Run /wsf-new-project to set up.',
...
function extractSectionContent(fileContent, sectionName) {
  const startMarker = `<!-- WSF:${sectionName}-start`;
  const endMarker = `<!-- WSF:${sectionName}-end -->`;
...
function buildSection(sectionName, sourceFile, content) {
  return [
    `<!-- WSF:${sectionName}-start source:${sourceFile} -->`,
    content,
    `<!-- WSF:${sectionName}-end -->`,
  ].join('\n');
}
...
function generateSkillsSection(cwd) {
  ...
  for (const dir of SKILL_SEARCH_DIRS) {
    ...
    for (const entry of entries) {
      ...
      // Skip WSF's own installed skills — only surface project-specific skills
      if (entry.name.startsWith('wsf-')) continue;
...
function cmdWriteProfile(cwd, options, raw) {
  ...
  if (!outputPath) {
    outputPath = path.join(os.homedir(), '.claude', 'wsf', 'USER-PROFILE.md');
...
function cmdGenerateDevPreferences(cwd, options, raw) {
  ...
  if (analysis.data_source === 'questionnaire') {
    stackBlock = 'Stack preferences not available (questionnaire-only profile). Run `/wsf-profile-user --refresh` with session data to populate.';
  ...
  let outputPath = options.output;
  if (!outputPath) {
    outputPath = path.join(os.homedir(), '.claude', 'commands', 'wsf', 'dev-preferences.md');
  } else if (!path.isAbsolute(outputPath)) {
    outputPath = path.join(cwd, outputPath);
  }
  ...
  const result = {
    ...
    command_name: '/wsf-dev-preferences',
...
function cmdGenerateClaudeProfile(cwd, options, raw) {
  ...
  const sectionLines = [
    ...
    '## Developer Profile',
    '',
    `> Generated by WSF from ${dataSource}. Run \`/wsf-profile-user --refresh\` to update.`,
    '',
  ...
  if (fs.existsSync(targetPath)) {
    ...
    const startMarker = '<!-- WSF:profile-start -->';
    const endMarker = '<!-- WSF:profile-end -->';
...
function cmdGenerateClaudeMd(cwd, options, raw) {
  ...
  const sectionHeadings = {
    ...
    workflow: '## WSF Workflow Enforcement',
  };
  ...
  if (existingContent === null) {
    ...
    for (const name of MANAGED_SECTIONS) {
      ...
      const hasMarkers = fileContent.indexOf(`<!-- WSF:${name}-start`) !== -1;
    ...
    if (!options.auto && fileContent.indexOf('<!-- WSF:profile-start') === -1) {
  ...
  let profileStatus;
  if (finalContent && finalContent.indexOf('<!-- WSF:profile-start') !== -1) {
    if (action === 'created' || existingContent.indexOf('<!-- WSF:profile-start') === -1) {
  ...
  if (profileStatus === 'placeholder_added') message += ' Run /wsf-profile-user to unlock Developer Profile.';
