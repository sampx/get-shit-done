...
async function cmdExtractMessages(projectArg, options, raw, overridePath) {
  ...
  reapStaleTempFiles('wsf-pipeline-', { dirsOnly: true });
  const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'wsf-pipeline-'));
...
async function cmdProfileSample(overridePath, options, raw) {
  ...
  reapStaleTempFiles('wsf-profile-', { dirsOnly: true });
  const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'wsf-profile-'));
