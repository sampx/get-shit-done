...
function stateReplaceFieldWithFallback(content, primary, fallback, value) {
  ...
  process.stderr.write(
    `[wsf-tools] WARNING: STATE.md field "${primary}"${fallback ? ` (fallback: "${fallback}")` : ''} not found — update skipped. ` +
...
function buildStateFrontmatter(bodyContent, cwd) {
  ...
  const fm = { wsf_state_version: '1.0' };
...
/**
 * Write a WAITING.json signal file when WSF hits a decision point.
 * External watchers (fswatch, polling, orchestrators) can detect this.
 * File is written to .planning/WAITING.json (or .wsf/WAITING.json if .wsf exists).
 * Fixes #1034.
 */
function cmdSignalWaiting(cwd, type, question, options, phase, raw) {
  const wsfDir = fs.existsSync(path.join(cwd, '.wsf')) ? path.join(cwd, '.wsf') : planningDir(cwd);
  const waitingPath = path.join(wsfDir, 'WAITING.json');
...
function cmdSignalResume(cwd, raw) {
  const paths = [
    path.join(cwd, '.wsf', 'WAITING.json'),
