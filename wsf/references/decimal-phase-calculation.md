...
```bash
DECIMAL_PHASE=$(node "$HOME/.claude/wsf/bin/wsf-tools.cjs" phase next-decimal "${AFTER_PHASE}" --pick next)
BASE_PHASE=$(node "$HOME/.claude/wsf/bin/wsf-tools.cjs" phase next-decimal "${AFTER_PHASE}" --pick base_phase)
```
