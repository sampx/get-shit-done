...
AGENT_SKILLS_CHECKER=$(node "$HOME/.claude/wsf/bin/wsf-tools.cjs" agent-skills wsf-integration-checker 2>/dev/null)
```
...
```bash
integration_checker_model=$(node "$HOME/.claude/wsf/bin/wsf-tools.cjs" resolve-model wsf-integration-checker --raw)
```
...
node "$HOME/.claude/wsf/bin/wsf-tools.cjs" phases list
```
...
PHASE_INFO=$(node "$HOME/.claude/wsf/bin/wsf-tools.cjs" find-phase 01 --raw)
...
for summary in .planning/phases/*-*/*-SUMMARY.md; do
  [ -e "$summary" ] || continue
  node "$HOME/.claude/wsf/bin/wsf-tools.cjs" summary-extract "$summary" --fields requirements_completed --pick requirements_completed
done
```
...
```bash
NYQUIST_CONFIG=$(node "$HOME/.claude/wsf/bin/wsf-tools.cjs" config-get workflow.nyquist_validation --raw 2>/dev/null)
```
