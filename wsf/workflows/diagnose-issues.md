...
```bash
AGENT_SKILLS_DEBUGGER=$(node "$HOME/.claude/wsf/bin/wsf-tools.cjs" agent-skills wsf-debugger 2>/dev/null)
...
```bash
node "$HOME/.claude/wsf/bin/wsf-tools.cjs" commit "docs({phase_num}): add root causes from diagnosis" --files ".planning/phases/XX-name/{phase_num}-UAT.md"
```
</step>
