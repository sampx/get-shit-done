...
```bash
INIT=$(node "$HOME/.claude/wsf/bin/wsf-tools.cjs" docs-init)
if [[ "$INIT" == @file:* ]]; then INIT=$(cat "${INIT#@file:}"); fi
AGENT_SKILLS=$(node "$HOME/.claude/wsf/bin/wsf-tools.cjs" agent-skills wsf-doc-writer 2>/dev/null)
```
...
```bash
node "$HOME/.claude/wsf/bin/wsf-tools.cjs" commit "docs: generate project documentation" \
