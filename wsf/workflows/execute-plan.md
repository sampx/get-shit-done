...
@~/.claude/wsf/references/git-integration.md
</required_reading>
...
```bash
INIT=$(node "$HOME/.claude/wsf/bin/wsf-tools.cjs" init execute-phase "${PHASE}")
...
```bash
node "$HOME/.claude/wsf/bin/wsf-tools.cjs" phases list --type summaries --raw
...
See `~/.claude/wsf/references/tdd.md` for structure.
</tdd_plan_execution>
...
```bash
node ~/.claude/wsf/bin/wsf-tools.cjs commit-to-subrepo "{type}({phase}-{plan}): {description}" --files file1 file2 ...
```
...
See ~/.claude/wsf/references/checkpoints.md for details.
</step>
...
```bash
NODE_REPAIR=$(node "./.claude/wsf/bin/wsf-tools.cjs" config-get workflow.node_repair 2>/dev/null || echo "true")
```

If `NODE_REPAIR` is `true`: invoke `@./.claude/wsf/workflows/node-repair.md` with:
...
If user_setup exists: create `{phase}-USER-SETUP.md` using template `~/.claude/wsf/templates/user-setup.md`. Per service: env vars table, account setup checklist, dashboard config, local dev notes, verification commands. Status "Incomplete". Set `USER_SETUP_CREATED=true`. If empty/missing: skip.
</step>

<step name="create_summary">
Create `{phase}-{plan}-SUMMARY.md` at `.planning/phases/XX-name/`. Use `~/.claude/wsf/templates/summary.md`.
...
node "$HOME/.claude/wsf/bin/wsf-tools.cjs" state advance-plan

# Recalculate progress bar from disk state
node "$HOME/.claude/wsf/bin/wsf-tools.cjs" state update-progress

# Record execution metrics
node "$HOME/.claude/wsf/bin/wsf-tools.cjs" state record-metric \
...
node "$HOME/.claude/wsf/bin/wsf-tools.cjs" state add-decision \
  --phase "${PHASE}" --summary-file "${DECISION_TEXT_FILE}" --rationale-file "${RATIONALE_FILE}"

# Add blockers if any found
node "$HOME/.claude/wsf/bin/wsf-tools.cjs" state add-blocker --text-file "${BLOCKER_TEXT_FILE}"
```
</step>
...
```bash
node "$HOME/.claude/wsf/bin/wsf-tools.cjs" state record-session \
...
```bash
node "$HOME/.claude/wsf/bin/wsf-tools.cjs" roadmap update-plan-progress "${PHASE}"
```
...
```bash
node "$HOME/.claude/wsf/bin/wsf-tools.cjs" requirements mark-complete ${REQ_IDS}
```
...
```bash
node "$HOME/.claude/wsf/bin/wsf-tools.cjs" commit "docs({phase}-{plan}): complete [plan-name] plan" --files .planning/phases/XX-name/{phase}-{plan}-SUMMARY.md .planning/STATE.md .planning/ROADMAP.md .planning/REQUIREMENTS.md
```
</step>
