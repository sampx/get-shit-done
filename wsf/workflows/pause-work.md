...
```bash
timestamp=$(node "$HOME/.claude/wsf/bin/wsf-tools.cjs" current-timestamp full --raw)
```
...
<step name="commit">
```bash
node "$HOME/.claude/wsf/bin/wsf-tools.cjs" commit "wip: [context-name] paused at [X]/[Y]" --files [handoff-path] .planning/HANDOFF.json
```
</step>
