...
Key references:
- @$HOME/.claude/wsf/references/ui-brand.md (display patterns)
- @$HOME/.claude/wsf/agents/wsf-user-profiler.md (profiler agent definition)
- @$HOME/.claude/wsf/references/user-profiling.md (profiling reference doc)
</required_reading>
...
```bash
cp "$HOME/.claude/wsf/USER-PROFILE.md" "$HOME/.claude/USER-PROFILE.backup.md"
```
...
✓ Stores profile at $HOME/.claude/wsf/USER-PROFILE.md
...
Run session scan:
```bash
SCAN_RESULT=$(node $HOME/.claude/wsf/bin/wsf-tools.cjs scan-sessions --json 2>/dev/null)
```
...
```bash
SAMPLE_RESULT=$(node $HOME/.claude/wsf/bin/wsf-tools.cjs profile-sample --json 2>/dev/null)
```
...
- The user-profiling reference doc at `$HOME/.claude/wsf/references/user-profiling.md`
...
Reference: @$HOME/.claude/wsf/references/user-profiling.md
...
**Get questions:**
```bash
QUESTIONS=$(node $HOME/.claude/wsf/bin/wsf-tools.cjs profile-questionnaire --json 2>/dev/null)
```
...
```bash
ANALYSIS_RESULT=$(node $HOME/.claude/wsf/bin/wsf-tools.cjs profile-questionnaire --answers "$ANSWERS_PATH" --json 2>/dev/null)
```
...
```bash
node $HOME/.claude/wsf/bin/wsf-tools.cjs write-profile --input "$ANALYSIS_PATH" --json 2>/dev/null
```

Display: "✓ Profile written to $HOME/.claude/wsf/USER-PROFILE.md"
...
**If no artifacts selected:** Display "No artifacts generated. Your profile is saved at $HOME/.claude/wsf/USER-PROFILE.md" and jump to step 10.
...
```bash
node $HOME/.claude/wsf/bin/wsf-tools.cjs generate-dev-preferences --analysis "$ANALYSIS_PATH" --json 2>/dev/null
```

Display: "✓ Generated /wsf-dev-preferences at $HOME/.claude/commands/wsf/dev-preferences.md"

**For CLAUDE.md profile section (if selected):**

```bash
node $HOME/.claude/wsf/bin/wsf-tools.cjs generate-claude-profile --analysis "$ANALYSIS_PATH" --json 2>/dev/null
```
...
```bash
node $HOME/.claude/wsf/bin/wsf-tools.cjs generate-claude-profile --analysis "$ANALYSIS_PATH" --global --json 2>/dev/null
```
...
Your profile:    $HOME/.claude/wsf/USER-PROFILE.md
```

Then list paths for each generated artifact:
```
Artifacts:
  ✓ /wsf-dev-preferences   $HOME/.claude/commands/wsf/dev-preferences.md
