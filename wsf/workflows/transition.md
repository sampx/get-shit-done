...
```bash
TRANSITION=$(node "$HOME/.claude/wsf/bin/wsf-tools.cjs" phase complete "${current_phase}")
```
...
```bash
PROGRESS=$(node "$HOME/.claude/wsf/bin/wsf-tools.cjs" progress bar --raw)
```
...
if [ -n "$WSF_WORKSTREAM" ]; then
  WS_LIST=$(node "$HOME/.claude/wsf/bin/wsf-tools.cjs" workstream list --raw)
fi
```
...
```bash
node "$HOME/.claude/wsf/bin/wsf-tools.cjs" config-set workflow._auto_chain_active false
```
