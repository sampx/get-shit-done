...
AGENT_SKILLS_UI=$(node "$HOME/.claude/wsf/bin/wsf-tools.cjs" agent-skills wsf-ui-researcher 2>/dev/null)
AGENT_SKILLS_UI_CHECKER=$(node "$HOME/.claude/wsf/bin/wsf-tools.cjs" agent-skills wsf-ui-checker 2>/dev/null)
```
...
```bash
UI_RESEARCHER_MODEL=$(node "$HOME/.claude/wsf/bin/wsf-tools.cjs" resolve-model wsf-ui-researcher --raw)
UI_CHECKER_MODEL=$(node "$HOME/.claude/wsf/bin/wsf-tools.cjs" resolve-model wsf-ui-checker --raw)
```

Check config:

```bash
UI_ENABLED=$(node "$HOME/.claude/wsf/bin/wsf-tools.cjs" config-get workflow.ui_phase 2>/dev/null || echo "true")
```
...
```markdown
Read ~/.claude/agents/wsf-ui-researcher.md for instructions.
...
Template: ~/.claude/wsf/templates/UI-SPEC.md
</output>
...
```markdown
Read ~/.claude/agents/wsf-ui-checker.md for instructions.
...
```bash
node "$HOME/.claude/wsf/bin/wsf-tools.cjs" commit "docs(${padded_phase}): UI design contract" --files "${PHASE_DIR}/${PADDED_PHASE}-UI-SPEC.md"
```
