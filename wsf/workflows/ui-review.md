...
AGENT_SKILLS_UI_REVIEWER=$(node "$HOME/.claude/wsf/bin/wsf-tools.cjs" agent-skills wsf-ui-reviewer 2>/dev/null)
```

Parse: `phase_dir`, `phase_number`, `phase_name`, `phase_slug`, `padded_phase`, `commit_docs`.

```bash
UI_AUDITOR_MODEL=$(node "$HOME/.claude/wsf/bin/wsf-tools.cjs" resolve-model wsf-ui-auditor --raw)
```
...
```markdown
Read ~/.claude/agents/wsf-ui-auditor.md for instructions.
...
```bash
node "$HOME/.claude/wsf/bin/wsf-tools.cjs" commit "docs(${padded_phase}): UI audit review" --files "${PHASE_DIR}/${PADDED_PHASE}-UI-REVIEW.md"
```
