<purpose>
Check for WSF updates via npm, display changelog for versions between installed and latest, obtain user confirmation, and execute clean installation with cache clearing.
</purpose>
...
Detect whether WSF is installed locally or globally by checking both locations and validating install integrity.

First, derive `PREFERRED_CONFIG_DIR` and `PREFERRED_RUNTIME` from the invoking prompt's `execution_context` path:
- If the path contains `/wsf/workflows/update.md`, strip that suffix and store the remainder as `PREFERRED_CONFIG_DIR`
...
Use `PREFERRED_RUNTIME` as the first runtime checked so `/wsf-update` targets the runtime that invoked it.
...
# runtime directories.
if [ -n "$PREFERRED_CONFIG_DIR" ] && { [ -f "$PREFERRED_CONFIG_DIR/wsf/VERSION" ] || [ -f "$PREFERRED_CONFIG_DIR/wsf/workflows/update.md" ]; }; then
  ...
  if [ -f "$PREFERRED_CONFIG_DIR/wsf/VERSION" ] && grep -Eq '^[0-9]+\.[0-9]+\.[0-9]+' "$PREFERRED_CONFIG_DIR/wsf/VERSION"; then
    INSTALLED_VERSION="$(cat "$PREFERRED_CONFIG_DIR/wsf/VERSION")"
  else
...
for entry in "${ORDERED_RUNTIME_DIRS[@]}"; do
  runtime="${entry%%:*}"
  dir="${entry#*:}"
  if [ -f "./$dir/wsf/VERSION" ] || [ -f "./$dir/wsf/workflows/update.md" ]; then
    LOCAL_RUNTIME="$runtime"
    LOCAL_VERSION_FILE="./$dir/wsf/VERSION"
    LOCAL_MARKER_FILE="./$dir/wsf/workflows/update.md"
...
for entry in "${ORDERED_ENV_RUNTIME_DIRS[@]}"; do
  ...
  if [ -f "$dir/wsf/VERSION" ] || [ -f "$dir/wsf/workflows/update.md" ]; then
    ...
    GLOBAL_VERSION_FILE="$dir/wsf/VERSION"
    GLOBAL_MARKER_FILE="$dir/wsf/workflows/update.md"
...
if [ -z "$GLOBAL_RUNTIME" ]; then
  for entry in "${ORDERED_RUNTIME_DIRS[@]}"; do
    runtime="${entry%%:*}"
    dir="${entry#*:}"
    if [ -f "$HOME/$dir/wsf/VERSION" ] || [ -f "$HOME/$dir/wsf/workflows/update.md" ]; then
      GLOBAL_RUNTIME="$runtime"
      GLOBAL_VERSION_FILE="$HOME/$dir/wsf/VERSION"
      GLOBAL_MARKER_FILE="$HOME/$dir/wsf/workflows/update.md"
...
```bash
npm view wsf-cc version 2>/dev/null
```
...
To update manually: `npx wsf-cc --global`
```
...
⚠️  **Note:** The installer performs a clean install of WSF folders:
- `commands/wsf/` will be wiped and replaced
- `wsf/` will be wiped and replaced
- `agents/wsf-*` files will be replaced
...
- Custom commands not in `commands/wsf/` ✓
- Custom agents not prefixed with `wsf-` ✓
- Custom hooks ✓
- Your CLAUDE.md files ✓

If you've modified any WSF files directly, they'll be automatically backed up to `wsf-local-patches/` and can be reapplied with `/wsf-reapply-patches` after the update.
```
...
**If LOCAL install:**
```bash
npx -y wsf-cc@latest "$RUNTIME_FLAG" --local
```

**If GLOBAL install:**
```bash
npx -y wsf-cc@latest "$RUNTIME_FLAG" --global
```

**If UNKNOWN install:**
```bash
npx -y wsf-cc@latest --claude --global
```
...
for dir in "${CACHE_DIRS[@]}"; do
  if [ -n "$dir" ]; then
    rm -f "$dir/cache/wsf-update-check.json"
  fi
done

for dir in .claude .config/opencode .opencode .gemini .config/kilo .kilo .codex; do
  rm -f "./$dir/cache/wsf-update-check.json"
  rm -f "$HOME/$dir/cache/wsf-update-check.json"
done
```

The SessionStart hook (`wsf-check-update.js`) writes to the detected runtime's cache directory, so preferred/env-derived paths and default paths must all be cleared to prevent stale update indicators.
</step>
...
```
╔═══════════════════════════════════════════════════════════╗
║  WSF Updated: v1.5.10 → v1.5.15                           ║
╚═══════════════════════════════════════════════════════════╝

⚠️  Restart your runtime to pick up the new commands.

[View full changelog](https://github.com/wsf-build/wsf/blob/main/CHANGELOG.md)
```
</step>
...
Check for wsf-local-patches/backup-meta.json in the config directory.

**If patches found:**

```
Local patches were backed up before the update.
Run /wsf-reapply-patches to merge your modifications into the new version.
```
